...
describe('цвет различий — постоянная шкала, а подпись говорит «насколько»', () => {
  ...
  it('подпись говорит, насколько пострадала худшая деталь', async () => {
    // Цвет отвечает «где», подпись — «насколько». Без неё слабое сжатие неотличимо от
    // никакого: и там и там модель зелёная.
    await показать('#020202', '#040404')
    const доля = viewer.diffScale()
    // 0x04 = 4 из 255 ≈ 1,6%.
    expect(доля, 'подписи нет — «тронуло на процент» неотличимо от «не тронуло»').toBeGreaterThan(0.01)
    expect(доля, 'подпись завышена: столько модель не менялась').toBeLessThan(0.02)
  })

  it('ничего не изменилось — подпись это и говорит', async () => {
    await показать('#000000', '#000000')
    expect(viewer.diffScale(), 'при полном совпадении худшее изменение равно нулю').toBe(0)
    const c = цвет('Сильная')
...
describe('пропавшие детали считаются потерей наравне с цветом', () => {
  ...
  it('добавленная резкость весит меньше, чем потерянная', () => {
    // Считаем только УБЫЛЬ контраста. Если деталей стало БОЛЬШЕ (бывает у кодеков с
    // резкостью), это не потеря, и вклада в красноту она давать не должна — остаётся один
    // сдвиг цвета, тот же в обе стороны.
    ...
    expect(прибавилось.красный, `прибавилось ${прибавилось.красный}, пропало ${пропало.красный}: `
      + 'прибавка деталей засчитана наравне с потерей').toBeLessThan(пропало.красный - 20)
  })
})
