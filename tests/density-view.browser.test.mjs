...
describe('в счёт идут все карты, а не только базовый цвет', () => {
  ...
  it('слоты УСРЕДНЯЮТСЯ: одна испорченная карта из двух краснит слабее, чем обе', () => {
    ...
    // Сравниваем по ЗЕЛЁНОМУ, а не по красному, и вот почему. Палитра идёт зелёный →
    // жёлтый → красный: во второй половине шкалы красный уже полный у обоих случаев, и
    // разницу несёт остаток зелени. Меньше зелени — хуже. Свойство то же самое: «две
    // потери сильнее одной», — измеряется только там, где оно теперь живёт.
    expect(обе.зелёный, `одна ${одна.зелёный}, обе ${обе.зелёный} зелени: две потери обязаны `
      + 'выглядеть хуже одной').toBeLessThan(одна.зелёный)
  })
...
describe('цвет различий — постоянная шкала, а подпись говорит «насколько»', () => {
  ...
  it('мелкое изменение остаётся ЗЕЛЁНЫМ, а не краснеет', async () => {
    // Ровно то, на чём откатили. Отклонения 2% и 7% — это «почти цело», и выглядеть они
    // обязаны соответственно, в какой бы модели ни встретились.
    await показать('#040404', '#111111')
  ...
  it('подпись говорит, насколько пострадала худшая деталь', async () => {
    ...
    // 0x11 = 17 из 255 ≈ 6,7%.
    expect(доля, 'подписи нет — «тронуло на процент» неотличимо от «не тронуло»').toBeGreaterThan(0.05)
    expect(доля, 'подпись завышена: столько модель не менялась').toBeLessThan(0.08)
  })
