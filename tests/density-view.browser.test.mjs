...
describe('карта различий ложится ТУДА ЖЕ, куда лежала текстура', () => {
  ...
  it('одни карты, разное размещение — одна запись в памяти и своя копия каждому', async () => {
    // ТОНКОЕ МЕСТО, и первая редакция решила его неверно: положила размещение в ключ
    // памяти. У анимированной текстуры сдвиг меняется каждый кадр — ключ вместе с ним, и
    // модель пересчитывалась заново при каждом входе в режим (Александр 2026-09-04: «с
    // анимированными текстурами всё очень плохо»).
    //
    // Ключ отвечает на вопрос «то же ли СОДЕРЖИМОЕ». Размещение не меняет ни одного
    // пикселя, поэтому запись в памяти одна, а спор двух материалов за неё решается копией.
    ...
    // А размещение у двух материалов своё, и общая карта его не перепутает.
  ...
  it('сдвиг анимации не заставляет считать заново', async () => {
    // Прямое следствие того же: вошли, вышли, сдвиг уехал, вошли снова — расчёта быть не
    // должно, карта та же самая.
    const model = модельСРазмещением((t) => { t.offset.set(0, 0) })
    ...
    let исходная = null
    model.traverse((o) => { if (o.isMesh) исходная = viewer._родной(o)?.map })
    исходная.offset.set(0.42, 0.17)          // анимация сдвинула развёртку
...
describe('карта различий совпадает с исходной по МЕСТУ, а не только по развёртке', () => {
  // Размещение (сдвиг, масштаб, набор) карта наследует — это проверено выше. Но остаётся
  // вопрос, который до сих пор не проверялся ни разу: совпадает ли она по ВЕРТИКАЛИ.
  //
  // Мест, где это может перевернуться, три, и все три в одном пути:
  //   - пиксели снимаются с видеокарты, а у неё начало координат СНИЗУ, у холста сверху;
  //   - готовая карта — `CanvasTexture`, у неё своё `flipY`;
  //   - у текстур из glTF `flipY` равен `false`, а у сделанных из холста — `true`.
  //
  // Перевёрнутая карта выглядит правдоподобно: цвета те же, распределение похожее. Именно
  // поэтому её надо ловить числом, а не глазами: «красное не там» человек заметит только на
  // модели с явной асимметрией, и то не сразу.

  /** Половинка: верх одного цвета, низ другого. Асимметрия, по которой видно переворот. */
  ...
  /** Средняя краснота верхней и нижней половин готовой карты. */
  ...
  for (const flipY of [true, false]) {
    it(`испорчен ВЕРХ — краснеет верх (flipY: ${flipY})`, () => {
      // Эталон ровный, у результата испорчена верхняя половина. Красное обязано лечь
      // туда же — иначе человек чинит не ту половину текстуры.
      const карта = viewer._diffTexture(
  ...
  it('готовая карта наследует flipY эталона', () => {
    // Иначе она ляжет вверх ногами уже на самой модели — при верном содержимом.
    for (const flipY of [true, false]) {
      const карта = viewer._diffTexture(
