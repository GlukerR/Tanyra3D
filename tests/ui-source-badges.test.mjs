...
describe('отбор отдельно, работа отдельно', () => {
  ...
  it('готовность меряется ЗНАЧЕНИЯМИ, а не числом нажатий', () => {
    // Здесь стоял счётчик правок, и он считал НАЖАТИЯ, а не разницу: поставил флажок и
    // снял обратно — настройки прежние, а весь пакет уходил на пересборку. Александр,
    // 2026-08-26: «я выбираю новый флажок и потом его убираю. и модели снова все нужно
    // полностью оптимизировать, хотя в левом вьюпорте уже висит оптимизированная по этим
    // же параметрам модель».
    const fn = block('function needsBuild(');
    expect(fn, 'счётчик нажатий сюда возвращать нельзя — он не видит возврата настройки')
      .not.toMatch(/settingsRevision|builtRevision/);
    expect(fn).toMatch(/settingsStamp\(\)/);
  });

  it('там, где будущие флажки ИЗВЕСТНЫ, сравниваются подписи', () => {
    // Два таких случая: модель на экране (её флажки видны) и любая модель в режиме
    // «Мой выбор» (флажки там ОБЩИЕ — соседу достанется ровно то, что стоит сейчас).
    // Это та же мера, которой живёт одиночная кнопка: разойдись они, и слова «уже
    // собрано» значили бы у пакета и у одиночки разное.
    const fn = block('function needsBuild(');
    expect(fn).toMatch(/if \(live \|\| adviceMode === 'manual'\) return signature !== currentSettingsSignature\(\)/);
  });

  it('в «Советуем» подпись соседа с текущей не сравнивается', () => {
    // Там панель собирается заново при каждом показе модели, и правка флажка до соседей
    // не доезжает. Сравни их подпись с текущей — и все соседи вечно «изменившиеся»: то
    // есть пропуск не сработает ни разу, а тесты останутся зелёными.
    const fn = block('function needsBuild(');
    const tail = fn.slice(fn.indexOf("adviceMode === 'manual'"));
    expect(tail).toMatch(/rec\.state\.builtStamp !== settingsStamp\(\)/);
  });

  it('в отпечаток входит только ОБЩЕЕ, флажков в нём нет', () => {
    // Отпечаток нужен ровно для соседей в «Советуем». Всунь в него подпись флажков — и
    // любая правка снова гнала бы весь пакет на пересборку, то есть дефект вернулся бы.
    const fn = block('function settingsStamp()');
    expect(fn).toMatch(/platformSelect\.value/);
    expect(fn).toMatch(/engineSelect\.value/);
    expect(fn, 'сам режим — тоже настройка человека, его переключение старит собранное')
      .toMatch(/\$\{adviceMode\}/);
    expect(fn, 'флажкам в общем отпечатке не место — см. needsBuild')
      .not.toMatch(/currentSettingsSignature|getSelectedFeatures/);
  });
