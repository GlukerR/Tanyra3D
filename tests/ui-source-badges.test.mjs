...
describe('отбор отдельно, работа отдельно', () => {
  ...
  it('кодек площадки доезжает до интерфейса от сервера', () => {
    // Значение живёт в профиле (baselineOpts.codec) и обязано пройти проверку: профиль
    // правят руками, а непроверенное значение не совпало бы ни с одной радиокнопкой —
    // результат верный, экран пустой.
    expect(SERVER).toMatch(/defaults: \{ texMode: advisedTexMode, codec: advisedCodec \}/);
    expect(SERVER).toMatch(/\['meshopt', 'draco', 'quantize'\]\.includes\(planDefaults\.codec\)/);
  });
