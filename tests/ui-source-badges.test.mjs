...
// Контракт движка и объявление глобального лица браузерного слоя: между ними есть
// НАМЕРЕННЫЙ дубль, и раздел 6 сторожит именно его.
...
// ═══════════════════════════════════════════════════════════════════════════
// 7. Список способов показа — один факт, и намеренная копия под присмотром
// ═══════════════════════════════════════════════════════════════════════════
//
// Ревизия 2026-09-01, проход П2. Список «сетка / глина / материалы» был написан руками
// в ЧЕТЫРЁХ местах: союзом в `contract.ts`, массивом в `viewer.ts`, цепочкой сравнений
// внутри проверки в `index.ts` и ещё раз в `ui/globals.d.ts`. Четвёртый способ показа
// пришлось бы дописать в каждое, и забытое место дало бы не ошибку сборки, а тихо
// неработающую кнопку.
//
// Три копии сведены к одной: список живёт в `contract.ts`, тип выводится из него,
// остальные ввозят. Четвёртая — в `globals.d.ts` — УБРАНА БЫТЬ НЕ МОЖЕТ и это не
// упущение: браузерный слой работает без сборщика, связь там глобальная переменная, а не
// импорт (шапка того файла). Копию, которую нельзя убрать, ставят под сторож — иначе она
// разойдётся ровно так же, только молча.

describe('способы показа названы в одном месте', () => {
  /** Список из contract.ts — единственный источник. */
  const изКонтракта = () => {
  ...
  it('contract.ts объявляет список, а тип выводится из него', () => {
    // Тип, написанный руками рядом со списком, — это снова две копии, только соседние.
  ...
  it('никто, кроме contract.ts и globals.d.ts, не выписывает список руками', () => {
    // `viewer.ts` и `index.ts` обязаны ввозить. Литерал в них означает вернувшуюся копию.
