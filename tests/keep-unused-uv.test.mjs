  ...
  it('наш отбор совпадает с библиотечным с точностью до развёртки', async () => {
    ...
    for (const name of [...REPO_MODELS].sort()) {
      ...
      try {
        ...
        // Пропускаем ТОЛЬКО то, что не открыть по существу: нужен внешний декодер
        // (meshopt) либо файл битый нарочно. «Файла нет на диске» — совсем другое дело:
        // модель из списка разрешённых обязана быть в репозитории (это отдельно сторожит
        // tests/local-model-guard.test.mjs), и молча считать её проверенной нельзя.
        if (e?.code === 'ENOENT' || !fs.existsSync(modelPath(name))) throw e;
        continue;
      }

      const наш = fns.cloneDocument(исходный);
      // Считаем, сколько сняли: сторож, которому нечего сравнивать, зелен и бесполезен.
    ...
    // ДВА ПОРОГА, и оба нужны — поодиночке каждый пропускает свой вид пустоты.
    //
    // Первый: сколько моделей вообще прочиталось. Второй: нашлось ли на них хоть что-то
    // лишнее. Без второго сторож зеленеет вхолостую — если из корпуса уйдут модели с
    // неиспользуемыми нормалями и касательными, обе стороны снимут по нулю, списки
    // совпадут, и любое расхождение нашей копии с библиотечной проедет незамеченным
    // (находка ревью 2026-09-01).
...
// ============================================================================
// 6. След просьбы виден при ЛЮБОЙ чистке, и ровно один раз.
// ============================================================================
// Находка ревью 2026-09-01. Строку «развёртка оставлена» писало только
// `structure/prune-unused`, включённое от `safe`. А развёртку сохраняет и
// `structure/prune-final` — оно включается ШИРЕ: от safe, склейки ИЛИ сжатия геометрии.
// Замер это подтвердил: meshopt без safe развёртку сохранял и молчал.
//
// Отсюда два утверждения, и второе не менее важно первого: след обязан БЫТЬ (Правило 12)
// и обязан быть ОДИН (Правило 9 — отчёт не толпа одинаковых строк).

describe('след просьбы в отчёте', () => {
  /** Сколько раз отчёт сказал «развёртка оставлена». */
  ...
  it('с safe и сжатием сразу — след всё равно ОДИН', async () => {
    // Оба правила чистки отработали. Скажи каждое своё — человек прочитал бы одно и то же
    // дважды и решил, что развёртку сохранили как-то по-особенному.
