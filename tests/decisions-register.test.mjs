...
describe('Свод решений правил', () => {
  ...
  it('удалять замысел автора могут РОВНО четыре места (узкое исключение Правила 11)', () => {
    ...
    // (настоящая раскраска вершин — ветка правила attributes/vertex-colors) и
    // interactivity/strip-dead (пометка нажатия у части, на которую в графе нет отклика).
    //
    // Четвёртое пришло 2026-08-29 по прямой просьбе Александра — «единственная галочка,
    // убрать нерабочий или лишний или пустой интерактив», — то есть разговор, которого
    // требует Правило 11, состоялся. Сторож при этом не ослаблен: он по-прежнему падает
    // на ПЯТОМ, появившемся молча.
    //
