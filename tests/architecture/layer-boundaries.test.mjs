 ...
 *
 * Второй появился 2026-08-28, в тот же день и по той же причине, что первый — только на
 * этот раз расхождение успело дойти до Александра. `core/interactivity-rules.mjs`
 * отвечает, что считается нажимаемой частью. Движок считал ЛЮБОЙ помеченный узел, вьюпорт
 * пропускал `"selectable": false`, и на `MagicBall` отчёт обещал 21 нажимаемую часть там,
 * где обводилась одна. Правило одно, спрашивают двое, данные у них разные — тот же шов.
 *
