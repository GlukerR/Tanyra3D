...
// ═══════════════════════════════════════════════════════════════════════════
// РАЗДЕЛ 5 · Развёртка переживает конвейер даже без текстур
// ═══════════════════════════════════════════════════════════════════════════

describe('развёртку у модели без текстур не выбрасываем', () => {
  // НАЙДЕНО 2026-08-22 на FBX Александра (экспортирован без материалов, карты лежали
  // отдельной папкой). Атрибуты до: NORMAL, POSITION, TEXCOORD_0. После: NORMAL, POSITION.
  //
  // Чистка считает атрибут лишним, если на него не ссылается ни один материал. Для модели
  // С текстурами это верно — восьмой UV-канал, которого не касается ни одна карта, мусор.
  // Но если текстур НЕТ ВОВСЕ, ссылаться нечему, и «не используется» значит «пока не
  // используется». Развёртку автор делал руками; выбросив её, мы отрезаем возможность
  // приложить к модели карты вообще когда-либо — ровно то, ради чего он просил ввоз FBX.
  //
  // Дефект был ДВОЙНОЙ, и это стоит помнить: правил его пришлось в двух местах. Первая
  // правка (structure/prune-unused) убрала строку из отчёта, а развёртку продолжала
  // уносить финальная чистка — то есть сделала потерю ТИХОЙ вместо того, чтобы убрать.
  // Замер после каждой правки, а не после первой, — единственное, что это показало.

  it('TEXCOORD_0 остаётся, когда текстур в модели нет', async () => {
    const { file } = makeFbx('keep-uv');
    const { doc } = await run(file);
    const prim = doc.getRoot().listMeshes()[0].listPrimitives()[0];
    expect(doc.getRoot().listTextures().length, 'заготовка неверна: текстур быть не должно').toBe(0);
    expect(prim.getAttribute('TEXCOORD_0'), 'развёртку выбросили — приложить карты станет нечем').toBeTruthy();
  });

  it('а лишний канал у модели С текстурами по-прежнему убирается', async () => {
    // Защита обязана быть УЗКОЙ. Сторож против обратной ошибки — «на всякий случай
    // оставим всё»: у модели с картами мёртвый канал остаётся мусором и должен уходить.
    // Проверяется на фотограмметрии (tests/photogrammetry.test.mjs): там 12 текстур,
    // две развёртки, и TEXCOORD_1 обязан исчезнуть. Здесь — только напоминание о связи.
    const { dir, file } = makeFbx('has-tex');
    await putTexture(dir);
    const { doc } = await run(file);
    expect(doc.getRoot().listTextures().length, 'текстура не доехала — тест ничего не проверяет').toBe(1);
    const prim = doc.getRoot().listMeshes()[0].listPrimitives()[0];
    expect(prim.getAttribute('TEXCOORD_0'), 'используемая развёртка пропала').toBeTruthy();
  });
});
