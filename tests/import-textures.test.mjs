...
// ═══════════════════════════════════════════════════════════════════════════
// ═══════════════════════════════════════════════════════════════════════════
// РАЗДЕЛ 5 · Таблица назначений объявлена ОДИН раз
// ═══════════════════════════════════════════════════════════════════════════

describe('таблица назначений карт объявлена один раз', () => {
  // ЧТО ЗДЕСЬ БЫЛО ДО 2026-08-26 И ПОЧЕМУ ПЕРЕПИСАНО.
  //
  // Таблица лежала ДВАЖДЫ — в аддоне и побайтно такая же в `ui/app.ts`, — а на страже
  // расхождения стоял тест, сверявший два ИСХОДНИКА построчно. В его шапке было
  // написано: «Слой интерфейса не имеет права импортировать addons/ … значит копия
  // неизбежна, и единственное, что делает её безопасной, это сторож на расхождение».
  //
  // Ограничение названо верно, вывод — нет. Копия оказалась избежима: данные ходят к
  // интерфейсу ЧЕРЕЗ ШОВ, и в том же самом ответе `/api/extensions` уже ехали группы
  // взаимоисключений, заведённые ровно по этой причине четырьмя месяцами раньше.
  // Прецедент был в том же файле, в том же запросе. Найдено аудитом, фаза Ф2.
  //
  // Правило починки из плана аудита звучит дословно так: не «синхронизировать», а
  // УДАЛИТЬ копию — синхронизация есть тот же дубль плюс обязанность помнить.
  //
  // ЧТО СТЕРЕЖЁМ ТЕПЕРЬ. Не совпадение двух текстов, а два утверждения:
  //   1. второй таблицы больше нет — интерфейс не объявляет своей;
  //   2. та, что едет по шву, ЖИВАЯ: собирается в рабочие регулярки и раскладывает
  //      имена файлов по назначениям.
  //
  // Проверка поведением, а не текстом, ещё и переживает переименование: сторож,
  // краснеющий от чистого переименования переменной, фаза Ф6 считает дефектом сторожа.

  const ROOT = path.resolve(path.dirname(fileURLToPath(import.meta.url)), '..');

  it('интерфейс не держит своей таблицы', () => {
    const ui = fs.readFileSync(path.join(ROOT, 'ui/app.ts'), 'utf8');
    // Признак копии — строка таблицы, а не имя переменной: назвать её могут как угодно.
  ...
  it('собранная из шва таблица раскладывает имена так же, как движок', async () => {
    // Сверяем РЕЗУЛЬТАТ на живых именах, а не форму данных. Имена взяты те, ради
    // которых таблица и существует, включая ловушку `_AO` внутри чужого слова.
    const { textureSlots } = await import('../optimize2.mjs');
