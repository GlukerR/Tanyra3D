...
describe('список принимаемых форматов живёт в ОДНОМ месте', () => {
  ...
  it('сервер не держит вторую копию списка расширений', () => {
    ...
    // Сообщение называет ЯКОРЬ. Сторож читает текст исходника и потому краснеет и на
    // чистом переименовании; прежний текст в этом случае утверждал «общего списка нет»,
    // то есть посылал контрибутора искать пропавшую фичу вместо правки образца
    // (аудит, фаза Ф6).
    expect(/const MODEL_EXT = /.test(SERVER),
      'в server.mts не найдено объявление `const MODEL_EXT = ` — общий список расширений '
      + 'либо переименован (поправь образец здесь), либо убран (тогда это дефект: '
      + 'проверка имени разъедется по местам)')
      .toBe(true);
  ...
  it('интерфейс и сервер принимают одно и то же', () => {
    ...
    // Оба сообщения называют ЯКОРЬ — см. разбор строкой выше по файлу.
    const ЯКОРЬ_UI = /const MODEL_RE = (\/[^/]+\/i)/;
    const ЯКОРЬ_СЕРВЕР = /const MODEL_EXT = (\/[^/]+\/i)/;
    const ui = APP.match(ЯКОРЬ_UI);
    expect(ui, `в ui/app.ts не найден фильтр форматов по образцу ${ЯКОРЬ_UI} — `
      + 'переименовали переменную (поправь образец) или убрали фильтр (это дефект)')
      .toBeTruthy();
    const server = SERVER.match(ЯКОРЬ_СЕРВЕР);
    expect(server, `в server.mts не найден фильтр форматов по образцу ${ЯКОРЬ_СЕРВЕР} — `
      + 'переименовали переменную (поправь образец) или убрали фильтр (это дефект)')
      .toBeTruthy();
...
describe('глина: показ безтекстурных моделей', () => {
  ...
  it('сама НЕ включается: новая модель показывается такой, какая в файле', () => {
    // ДЕФЕКТ 2026-08-22, нашёл Александр на своей `gluke-purple.glb`: «с моей модели на
    // которой был 1 цвет (фиолетовый) спал полностью этот цвет». Глина включалась сама у
    // всякой модели без текстур — на посылке «нет текстур, значит и цвета нет». У его
    // модели цвета не было только в этой посылке.
    //
    // И вторая причина, которая закрывает вопрос даже при точной посылке: глина — НАШ
    // способ смотреть, в файл она не попадает. Показывать ею по умолчанию значит
    // показывать не то, что человек получит на выходе.
    const m = INDEX.match(/_resetDisplayMaterial\(\)\s*\{[\s\S]*?\r?\n {2}\}/);
    expect(m, 'сброса показа нет').toBeTruthy();
    expect(/this\._display = 'file';/.test(m[0]), 'новая модель показывается не из файла').toBe(true);
    expect(/hasTextures\(\)\s*\?/.test(INDEX), 'догадка «нет текстур — значит глина» вернулась').toBe(false);
  });

  it('глина не стирает цвет автора', () => {
    // `MeshMatcapMaterial.color` умножается на картинку шара: форма от глины, цвет
    // авторский. У модели без цвета множитель белый — глина выходит ровно прежней.
    const m = VIEWER.match(/_clayFor\([\s\S]*?\r?\n {2}\}/);
    expect(m, 'глины для детали нет').toBeTruthy();
    expect(/color: hex/.test(m[0]), 'цвет автора в глину не передаётся').toBe(true);
    expect(/source/.test(m[0]), 'родной материал в глину не приходит').toBe(true);
  });
});
