...
describe('textures/resize — уменьшает только то, что крупнее цели', () => {
  ...
  // Замер 2026-08-12 (вопрос Александра про нестандартные размеры): по пропорции
  // выходило 1024×819, а toktx молча делал 820 — ему нужны блоки 4×4. Значит один и тот
  // же выбор давал два разных файла в зависимости от того, включён ли KTX2. Округляем
  // сами, и результат один при любом наборе флажков.
  ...
  it('никогда не увеличивает: 256 при цели 1024 остаётся 256', async () => {
    ...
    // Здесь до 2026-08-22 стояло «lines.length === 0» — «правило отчиталось о работе,
    // которой не было». Замысел верный, счёт устарел: коммит a8321a0 намеренно завёл
    // строку «включено, но в этой модели менять было нечего» (Правило 12 — показанная
    // галочка обязана отчитаться о том, что с ней стало). Ноль строк означал бы, что
    // человек выбрал размер и не узнал о своём выборе НИЧЕГО.
    //
    // Так что проверяем настоящий замысел: работой правило не хвастается, но и не молчит.
