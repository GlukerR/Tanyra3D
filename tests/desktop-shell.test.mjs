...
describe('падение движка НА ХОДУ оставляет чем разбираться', () => {
  ...
  // ДЕФЕКТ, найденный ревью 2026-08-22. Сбор вывода выключался сразу после удачного
  // запуска, а буфер обнулялся: «дальше сервер работает часами, и держать его журнал в
  // памяти незачем». Довод верный, вывод из него был неверный. Держать незачем ВЕСЬ
  // журнал — но не последние строки: когда движок умирает на ходу, они и есть
  // единственное, по чему можно понять причину.
  //
  // Чем это было для человека: «Движок остановился (код N)» — и всё. Файла
  // engine-crash.log при таком выходе не появлялось вовсе (он писался только при
  // неудачном ЗАПУСКЕ), а буфер к этому моменту был пуст. Рассказать о поломке нечем
  // ни ему, ни нам.
  ...
  it('буфер ограничен — иначе он растёт часами', () => {
    // Кольцо, а не «пусть копится»: программа работает долго, и вывод за смену — это
    // мегабайты, из которых нужны последние строки.
  ...
  it('при смерти на ходу пишется отчёт, а не только показывается код', () => {
    // Обработчик выхода ПОСЛЕ удачного запуска — тот, что заводится взамен сторожа
    // запуска (removeAllListeners). Именно ему и нужен отчёт.
...
describe('одна программа — один запущенный экземпляр', () => {
  ...
  it('второму экземпляру показывают окно первого, а не второе окно', () => {
    ...
    // ещё одно ПРИ ОТКРЫТОМ значило бы вернуть ровно ту беду, ради которой замок ставился.
    ...
    // Второй движок не поднимается ни при каком повороте — вот это и есть та самая беда.
  ...
  it('окно создаётся только когда его НЕТ — и на работающем движке', () => {
    // Уточнение к предыдущему (2026-08-21). Первая редакция запрещала createWindow в
    // обработчике вовсе — и запрещала слишком много: на macOS программа живёт без окон
    // (window-all-closed её не гасит), и там повторный запуск при закрытом окне получал
    // молчание вместо программы. Открыть окно на УЖЕ РАБОТАЮЩЕМ движке — это не второй
    // экземпляр, а ровно то же, что делает app.on('activate') при клике по значку в доке.
    //
    // Запрет остаётся в силе там, где он и был нужен: создавать окно поверх открытого
    // нельзя. Проверяем это по форме — вызов обязан стоять под проверкой `!mainWindow`
    // и под известным адресом движка.
    const handler = MAIN.match(/app\.on\('second-instance'[\s\S]*?\n {2}\}\);/)[0];
    ...
    if (/createWindow\(/.test(handler)) {
      ...
      // И ровно один вызов на весь обработчик: второй стоял бы уже при открытом окне.
