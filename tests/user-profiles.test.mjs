...
describe('площадка находится по id, а не по имени файла (аудит Ф4)', () => {
  // Заведено 2026-08-26 по находке Ф4-1. Ответов на вопрос «какие площадки есть» было
  // два: список читал `id` ВНУТРИ файла, загрузка искала файл ПО ИМЕНИ. Совпадали они
  // ровно до тех пор, пока имя файла равно id.
  //
  // Цена расхождения — не отказ, а ложь: площадка появлялась в выпадающем списке и
  // падала при выборе сообщением, которое само себе противоречило в одной строке:
  //   Unknown platform "авито". Available: google-store, shopify, vntana, авито.
  // Контрибутор по такому тексту ищет опечатку там, где её нет.
  //
  // ПРОБА НА КРАСНОТУ пройдена: вернул `profilePath` к поиску только по имени файла —
  // краснеют оба первых случая, третий (имя = id) остаётся зелёным.

  it('файл назван не по id — площадка и видна, и выбирается', () => {
    fs.writeFileSync(path.join(dir, 'моя-площадка.json'), JSON.stringify({ ...OZON, id: 'ozon' }), 'utf8');
    expect(listPlatforms('ru').map((p) => p.id), 'площадки нет в списке').toContain('ozon');
    expect(() => planFor('ozon', 'ru'), 'площадка есть в списке, но при выборе падает').not.toThrow();
  });

  it('id не латиницей — тоже находится', () => {
    // safeId() вырезает всё, кроме [a-z0-9_-], поэтому прямого пути по имени файла у
    // такого профиля нет вовсе — только поиск по id внутри файла.
    fs.writeFileSync(path.join(dir, 'avito.json'), JSON.stringify({ ...OZON, id: 'авито' }), 'utf8');
    expect(listPlatforms('ru').map((p) => p.id)).toContain('авито');
    expect(() => planFor('авито', 'ru'), 'кириллический id виден в списке, но не загружается').not.toThrow();
  });

  it('чего нет — того нет, и сообщение это не путает', () => {
    // Обратная сторона: поиск по содержимому не должен начать находить что попало.
    let msg = '';
    try { planFor('нет-такой-площадки', 'ru'); } catch (e) { msg = e.message; }
    expect(msg, 'несуществующая площадка загрузилась').toMatch(/Unknown platform/);
    const known = msg.slice(msg.indexOf('Available:'));
    expect(known, 'сообщение перечисляет ту самую площадку, которую назвало неизвестной')
      .not.toMatch(/нет-такой-площадки/);
  });
});
