...
export const LOCAL_ONLY = {
  // `skinned` УБРАН 2026-08-26 — третьим по счёту и по той же причине, что два ниже.
  // Класс держался на пяти ЧУЖИХ и клиентских моделях (RiggedSimple, parkergirl,
  // Lilith Character 01, chibi_zenitsu, Cthulhu Stone 01), и ни одна в git не едет.
  // Значит сторож `quantizationVolume: 'scene'` — тот, что закрывает июльский дефект
  // «meshopt размножает скин 1 → 14», — на чистом клоне и на CI не проверял НИКТО.
  //
  // Теперь у класса есть свой представитель: Skinned Morphs 01 (7.6 КБ, Apache-2.0,
  // генератор — _work/make-skin-morph.mjs). Три меша на ОДНОМ скине, разного размера и
  // разнесённые: без этого дефект не воспроизводится вовсе — у одного меша нет второй
  // области квантования, с которой могло бы разойтись преобразование. Проверено пробой:
  // со снятым сторожем заготовка даёт скин 1 → 3 и статус `fail`.
  //
  // Пять локальных моделей никуда не делись и остаются полезными — на них класс шире и
  // злее. Но класс больше НЕ ДЕРЖИТСЯ на них одних, а это и есть смысл объявления.
  //
