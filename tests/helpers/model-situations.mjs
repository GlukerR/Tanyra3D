...
export const KNOWN_HOLES = {
  'no-geometry': 'в корпусе нет модели с 0 треугольников: Orphan Texture Cube 01 имеет 12 (расхождение с названием «без геометрии» описано в отчёте quantize)',
  'textures-only': 'нет модели с текстурами и 0 треугольников',
  'pre-ktx2': 'нет текстур image/ktx2 на входе (фикстуры с KTX2 в корпус не добавляем)',
  'multi-scene': 'нет модели с ≥2 сценами',
  heavy: 'нет модели >50 МБ: самая большая ABeautifulGame 42.9 МБ; тяжёлые модели живут в input/ и у клиентов, в fixtures не добавляем',
};
