...
'option.safe.title': () => 'Safe optimizations',
  ...
  'option.keep-unused-uv.description': () => 'Cleanup drops a UV layout no image uses — it only takes up space. Untick this when the finish is chosen on the site itself: a configurator for furniture, clothing or surfaces. The UV then survives the build, and everything else unused is still cleaned away as usual.',
  'option.keep-unused-uv.impact': () => 'The file gets heavier by the weight of the UV itself: a fraction of a percent up to 20% in our measurements — the fewer images a model has, the more it shows.',
