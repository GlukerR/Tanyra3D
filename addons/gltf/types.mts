...
export type GltfOpts = {
   ...
   *
   * ЦЕНА ИЗМЕРЕНА, и она не мелочь. Библиотека даёт одну ручку на ВСЕ атрибуты сразу,
   * поэтому вместе с развёрткой остаются нормали, касательные и лишние каналы развёртки.
   * Замер 2026-08-29: `parkergirl` (материал unlit, нормали ему не нужны) — 4,60 МБ
   * против 7,59 МБ, то есть +65%; `Dirty Cube 01` (пять лишних каналов и касательные) —
   * +14%. Число названо человеку в «чем платишь» у самой опции.
   *
   * Отсюда очевидная следующая работа, если цена окажется дорогой: убирать точечно —
   * оставлять развёртку, но сносить нормали у unlit и неиспользуемые касательные. Это
   * значит считать «что читает материал» самим, вместо библиотеки, и потому отложено:
   * своя копия такого счёта разойдётся с библиотечной молча.
   */
