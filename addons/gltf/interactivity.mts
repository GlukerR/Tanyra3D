...
export interface Interactivity {
  ...
  clickable: number;
  /**
   * Откликов на действие человека в графе — нажатие и наведение вместе.
   *
   * ИМЕННО «ОТКЛИКОВ», а не «откликов на нажатие»: `onHover` считается здесь наравне с
   * `onSelect`, и слово «нажатие» покрывало бы наведение враньём. Замер 2026-08-31: ни
   * в одной модели корпуса `onHover` не встречается ни разу, то есть расхождение пока
   * теоретическое, — но число и его подпись обязаны сходиться до того, как такая модель
   * появится, а не после.
   *
   * ОКНО НАВЕДЕНИЕ НЕ ПРОИГРЫВАЕТ, и это осознанно. Узла `event/onHover` нет в списке
   * известных вычислителю, поэтому граф с ним отвергается ЦЕЛИКОМ и окно честно говорит
   * «только обводка» (Правило 12: показанная кнопка обязана работать). Учить окно
   * наведению без единой модели, на которой это проверить, значило бы писать вслепую —
   * и платить за это лучом по всей сцене на каждом движении мыши.
   */
  handlers: number;
  /** Запусков анимации. */
  animations: number;
