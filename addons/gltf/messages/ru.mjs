...
// Единицы по величине: «+0.0 МБ» на мелкой модели читается как поломка.
...
export default {
  ...
  'join.expandedShared': ({ bytes, pct, dcSaved }) =>
    `Объединение размножило общую геометрию: +${size(bytes)} (+${pct}%) хранимых данных ради ${dcSaved} отрисовок меньше. `
  ...
  'webp.keptOriginal': ({ n }) => `Текстур оставлено в исходном формате: ${n} — WebP на них оказался тяжелее, а выигрыш у WebP только в размере файла`,
