...
export default {
  ...
  'webp.sourceQuality': ({ q, exact }) => (exact
    ? `Качество исходных текстур: ${q} — с ним и закодированы, выше исходника не поднять`
    : `Качество исходных текстур: примерно ${q} — с ним и закодированы, выше исходника не поднять`),
  'webp.sourceQuality.range': ({ min, max, exact }) => (exact
    ? `Качество исходных текстур: от ${min} до ${max} — каждая закодирована со своим, выше исходника не поднять`
    : `Качество исходных текстур: примерно от ${min} до ${max} — каждая закодирована со своим, выше исходника не поднять`),
  ...
  // Формулировка нейтральная: 90 — рекомендуемое положение, то есть эта строка чаще
  // всего появляется БЕЗ всякого действия человека. Прежнее «по вашему выбору» было бы
  // при умолчании прямой неправдой.
