...
export const RULES: GltfRule[] = [
...

  {
    ...
    async fix(finding, ctx) {
      ...
      // Цена. Знак встаёт при ЛЮБОМ росте — порога «вдвое» здесь больше нет.
      //
      // ЧТО ПОКАЗАЛ ЗАМЕР 2026-08-18, включая опровержение первой версии этой правки.
      //
      // Порог стоял с рассуждением «небольшой рост — обычная плата, кричать о нём значит
      // приучить не читать». Снимать его я взялся, опираясь на собственный замер: будто
      // ABeautifulGame при ползунке 100 растёт на 21 %. ЭТО БЫЛО НЕВЕРНО. Замер делался
      // прямым вызовом кодировщика на quality:100, а правило так не работает — оно берёт
      // качество ИСХОДНИКА (у этой модели JPEG ≈83) и кодирует в него. Через само правило
      // модель УЖИМАЕТСЯ на 56 % (18 → 8 МБ картинок).
      //
      // Прогон всего корпуса через правило: НИ ОДНА модель не выросла вовсе. Ни на
      // проценты, ни вдвое. Синтетику тоже не удалось заставить вырасти — шумный JPEG
      // любого качества, случайный шум в PNG с альфой и без, крошечные заливки 4×4…64×64:
      // WebP выигрывает везде.
      //
      // Отсюда два вывода, и они тянут в разные стороны, поэтому записаны оба.
      //   1. Опасение про шум ничем не подтверждено: рост не случается, значит порог
      //      никогда не спасал от потока красных знаков — спасать было не от чего.
      //   2. Но и дефекта, который порог прятал бы, замерить не удалось. Эта правка —
      //      СТРАХОВКА на случай, которого сегодня в корпусе нет, а не починка найденной
      //      поломки. Тестом на настоящем кодировании она не закрыта и закрыта быть не
      //      может: рост нечем воспроизвести.
      //
      // Оставлено снятым по Правилу 12: человек включает галочку в том числе чтобы
      // ИЗМЕРИТЬ цену, и если рост однажды случится, молчание о нём сделает этот замер
      // такой же неправдой, как молчаливый откат. Цена ошибки в эту сторону — ноль
      // (нечему шуметь), в обратную — молчание там, где человек ждал числа.
      //
      ...
      if (vramBefore > 0 && vramAfter > vramBefore) {
        cost.push({
