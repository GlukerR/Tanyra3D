...
window.I18N_CATALOGS.ru = {
  ...
  // Общий выключатель над списком. Одна подпись на оба его положения намеренно: он
  // показывает состояние выбора, а не обещает действие (docs/ПРАВИЛА_ИНТЕРФЕЙСА.md).
  'batch.remove': 'Удалить отмеченные',
  'batch.remove.title': 'Удаление моделей',
  // Число — главное, что человек читает в этом окне: оно и есть повод передумать.
  ...
  // Пропущенные как уже собранные — ОДНА строка на всех (Правило 9). Молчать нельзя:
  // человек нажал кнопку и должен понимать, почему собралась не вся отмеченная пачка.
