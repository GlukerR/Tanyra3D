...
//
// Правило 6: повторяющаяся проверка «есть ли внутри модели лицензия» вынесена в скрипт,
// чтобы для каждой новой модели корпуса не делать это вручную. Живёт рядом с корпусом
// (а не в gitignored _work/), чтобы переживать сессии — в этом и цель Правила 6.
//
// Что делает:
