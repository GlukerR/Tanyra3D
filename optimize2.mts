...
//
// Вход: input/*.glb, *.gltf, *.stl, *.ply, *.fbx, *.obj → Выход: output/*.glb + *.report.md
// Список расширений один на всё приложение и живёт в аддоне (addons/gltf, поле formats).
//
// *Ревизия 2026-09-01: здесь было перечислено ЧЕТЫРЕ формата из шести — без `.fbx` и
// `.obj`, хотя принимаются они с 2026-08-19. Строка стоит вплотную к фразе «список живёт
// в аддоне», и это ровно та ШЕСТАЯ копия перечня, о которой предупреждает предложение 1
// аудита (`docs/АУДИТ_итог_предложения.md`): пять известных копий плюс эта, в тексте
// помощи. Человек, читающий помощь, не узнавал, что FBX можно просто передать.*
//
