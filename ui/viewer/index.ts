...
class DualViewport {
  ...
  constructor() {
    ...
    this._diffRefs = null;
    /**
     * Готовые карты различий, разложенные ПО ПАРАМ МОДЕЛЕЙ.
     *
     ...
     *
     * Ключ — исходник плюс адрес результата: пересборка МЕНЯЕТ адрес, значит сама собой
     * даёт новый ключ, а возврат к прежней паре находит посчитанное. Держим ТРИ пары:
     * человек ходит между несколькими моделями, а не между всеми сразу, и неограниченный
     * запас — это неограниченная память под холсты.
     */
  ...
  _applyDisplayMaterial() {
    ...
    if (mode === 'texdiff') {
      ...
      // Память по паре моделей ПОДСТАВЛЯЕТСЯ ПЕРВОЙ: `setDiffReference` ниже может сразу
      // применить режим, и к этому мгновению окно обязано держать карту нужной пары.
      // Карта памяти подставляется ВСЕГДА, даже когда устойчивого ключа нет.
      //
      // Память разложена по номеру детали, а номера у разных моделей совпадают. Не
      // подставь мы свежую карту — вторая модель нашла бы по номеру 0 карту ПЕРВОЙ и
      ...
      if (!this._diffKey) {
        ...
        if (!своя) {
          своя = new Map();
          // Три пары — потолок. Самая старая уходит вместе со своими холстами: держать всё
          // значило бы держать всю память под них, а это уже не память, а утечка.
  ...
  async loadOriginal(originalFile: File | null, pack?: PackEntry[] | null) {
    ...
    // Модель сменилась — прежние эталонные карты больше не о ней. Сами ПОСЧИТАННЫЕ карты
    // при этом остаются в `_diffStore`: вернётся эта пара — найдёт своё.
    this._diffRefs = null;
