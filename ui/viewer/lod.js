...
async function fromExtension(gltf) {
    ...
    if (!parser?.getDependency || !json?.nodes)
        return null;
    ...
    json.nodes.forEach((n, i) => {
        if (holder !== -1)
            return;
        const ext = n.extensions?.['MSFT_lod'];
        if (ext && Array.isArray(ext.ids) && ext.ids.length) {
            holder = i;
            ids = ext.ids;
        }
    });
    ...
    for (const nodeIndex of [holder, ...ids]) {
        const obj = (await parser.getDependency('node', nodeIndex));
        if (!obj)
            continue;
...
function fromSiblings(scene) {
    ...
    scene.traverse((parent) => {
        ...
        if (new Set(levels.map((l) => l.triangles)).size !== levels.length)
            return;
 ...
 */
export async function detectLods(gltf) {
    const byExtension = await fromExtension(gltf);
    if (byExtension)
        return byExtension;
