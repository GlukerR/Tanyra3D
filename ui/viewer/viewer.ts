 ...
 */
type DiffRaw = { data: Uint8Array; w: number; h: number; max: number; среднее: number; flipY: boolean };
...
export class Viewer implements ViewerLike {
  ...
  /**
   * Предел шкалы: насколько пострадала САМАЯ пострадавшая деталь этой пары моделей.
   * 0 — не изменилось ничего.
   */
  declare _diffMax: number;
  ...
  _diffRaw(эталоны: SlotMaps, ставшие: SlotMaps): DiffRaw | null {
    ...
    // ЧТО СЧИТАЕТСЯ ПОТЕРЕЙ. Не одно, а два, и второе добавлено 2026-09-04.
    //
    // Александр: «опять при смене размеров текстур 4 просто перестаёт меняться совсем».
    // Пересчёт при этом шёл — замер это показал, — но картинка почти не отличалась, и
    // причина оказалась в самой мере. Уменьшение 2048 → 512 меняет ПИКСЕЛИ всего на 2,7%
    // (`_work/detail-loss.mjs` на текстуре CarConcept): размытая картинка численно близка к
    // исходной. А ДЕТАЛЕЙ при этом исчезает 13,4% — впятеро больше.
    //
    // Глаз видит именно это: не сдвиг цвета, а пропавшую мелочь. Поэтому кроме отклонения
    // цвета считаем ПОТЕРЮ ЛОКАЛЬНОГО КОНТРАСТА — насколько ослаб перепад между соседними
    // пикселями. У целой картинки он равен нулю, поэтому «ничего не изменилось» остаётся
    // зелёным, как и требовал Александр в тот же день.
    //
    // Берём БОЛЬШЕЕ из двух, а не сумму: это два лица одной потери, и складывать их значит
    // считать одно и то же дважды там, где сжатие бьёт и по цвету, и по деталям сразу.
    const яркость = (a: Uint8ClampedArray, i: number) => (a[i]! + a[i + 1]! + a[i + 2]!) / 3;
    // Перепад до СЛЕДУЮЩЕГО пикселя, а не между соседями через одного.
    //
    // Симметричная разность (x+1 против x−1) слепа к узору с шагом в один пиксель: у
    // шахматки оба соседа одного цвета, и она выходит «ровной». А именно такую мелочь
    // уменьшение размера съедает первой — поймано собственным сторожем, который показал
    // два одинаковых числа там, где ждал разных.
    const контраст = (a: Uint8ClampedArray, x: number, y: number) => {
      if (x >= w - 1 || y >= h - 1) return 0;
      const i = (y * w + x) * 4;
      return Math.abs(яркость(a, i + 4) - яркость(a, i))
        + Math.abs(яркость(a, i + w * 4) - яркость(a, i));
    };
    ...
    for (let i = 0, p = 0; p < data.length; i += 4, p++) {
      ...
      for (const [a, b] of пары) {
        ...
        if (b) {
          // Потеря деталей — только УБЫЛЬ контраста. Прибавка (например, от резкости после
          // сжатия) потерей не является и красным помечаться не должна.
          const убыло = контраст(a, x, y) - контраст(b, x, y);
          if (убыло > d) d = Math.min(255, убыло);
        }
        // МАКСИМУМ ПО КАРТАМ, а не среднее.
        //
        // Слово Александра 2026-09-04: «не будем усреднять между картами, а будем брать
        // самое большое расхождение. Если металл никак не изменился, а нормалмапа
        // разошлась на 10%, мы и берём 10%».
        //
        // Усреднять просил он же 1 сентября — но тогда речь шла о том, чтобы нормаль вообще
        // УЧИТЫВАЛАСЬ, а не о том, чтобы она растворялась. Замер показал, чем это кончалось
        // (`_work/slot-stats.mjs`): у шахмат нормаль и шероховатость теряют по 3,6%, а цвет
        // с затенением почти целы — среднее по четырём картам давало 2,6%, и деталь
        // выходила зелёной. Нетронутая карта не должна разбавлять разнесённую.
        if (d > худшее) худшее = d;
      }
      // Среднее ПО КАРТАМ — по прямой просьбе Александра 2026-09-01: «нормаль тоже должна
      // учитываться, и значение должно усредняться». Внутри карты — максимум по каналам.
      const v = худшее;
      data[p] = v;
      всего += v;
      if (v > max) max = v;
    }
    return { data, w, h, max, среднее: всего / data.length, flipY: эталоны[слоты[0]!]!.flipY };
  }
  ...
  _diffColor(сырое: DiffRaw): THREE.CanvasTexture | null {
    ...
    for (let p = 0, i = 0; p < data.length; p++, i += 4) {
      const t = Math.min(1, data[p]! / (255 * Viewer.ПОЛНЫЙ_КРАСНЫЙ));
  ...
  _texdiffFor(родной: THREE.Material | null, эталон: SlotMaps, ключ: string) {
    ...
    // Насколько пострадала худшая деталь — это ПОДПИСЬ, а не шкала цвета. Цвет абсолютный.
    if (raw.среднее > (this._diffMax || 0)) this._diffMax = raw.среднее;
  ...
  /**
   * Насколько пострадала САМАЯ пострадавшая деталь: доля 0…1, `null` — сравнивать было
   * нечего. Не шкала цвета (та абсолютная), а ответ на вопрос «насколько вообще».
   */
  diffScale(): number | null {
    if (!this._diffCache.size) return null;
    return (this._diffMax || 0) / 255;
  }
  ...
  useDiffStore(store: Map<string, { raw: DiffRaw; tex: THREE.CanvasTexture }>) {
    ...
    // Предел принадлежит ПАРЕ моделей, а не окну: у другой пары он свой. Берём наибольший
    // из уже посчитанного в этой памяти — остальное уточнит расчёт.
    let max = 0;
    for (const я of store.values()) if (я.raw.среднее > max) max = я.raw.среднее;
    this._diffMax = max;
  }
