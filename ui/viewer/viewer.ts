...
/**
 * Эталонная деталь: ЕЁ ИМЯ и её карты.
 *
 * Имя здесь не украшение, а единственное, чем деталь узнают после сборки. Раньше пара
 * «эталон ↔ результат» держалась на порядковом номере в обходе, и это оказалось неверно:
 * сборка ПЕРЕСТАВЛЯЕТ детали. Замер на `ABeautifulGame` (`_work/pairs.mjs`): деталей
 * поровну, 49 и 49, а под номером 6 слева `Pawn_Top_W1`, справа уже `Pawn_Body_W2` —
 * 43 расхождения из 49. Верх пешки сравнивался с телом соседней.
 *
 * Так это и выглядело у Александра 2026-09-03: «на одинаковых пешках по-разному лежат
 * текстуры и ломаются по-разному… будто пешек не 2 вида, а 4».
 */
...
export class Viewer implements ViewerLike {
   ...
   * показана, а окно — нет.
   */
  _diffCache = new Map<string, THREE.CanvasTexture>();
  ...
  _diffTexture(эталоны: SlotMaps, ставшие: SlotMaps): THREE.CanvasTexture | null {
    ...
    for (let i = 0; i < карта.data.length; i += 4) {
      ...
      // СТЕПЕНЬ 0,4 — ВЫБРАНА ПО ЗАМЕРУ, а не на глаз.
      //
      // Александр 2026-09-03: «плохие участки не сильно помечаются, и получается зелёно-
      // красное всё; странно выглядит, особенно когда прям сильные расхождения».
      //
      // Замер объяснил, почему (`_work/diff-hist.mjs` на паре `ABeautifulGame`, WebP на
      // самом низком качестве): 93% пикселей отклоняются меньше чем на 5%, а сильнее 10% —
      // меньше процента. Потери настоящие, но мелкие: сжатие бережёт ровные места и рвёт
      // края. Прежний корень отдавал этому диапазону 0…0,32, то есть весь видимый ущерб
      // умещался в тусклую зелень, а половина шкалы не использовалась никогда.
      //
      // Степень 0,4 поднимает рабочий диапазон: 5% → 0,30, 10% → 0,40, 25% → 0,57. Потолок
      // при этом НЕ обрезается: «испорчена одна карта из двух» (0,76) по-прежнему отличимо
      // от «испорчены обе» (1,0) — различать их просил он же 2026-09-01.
      const t = Math.pow(сумма / пары.length / 255, 0.4);

      // ЗЕЛЁНЫЙ → ЖЁЛТЫЙ → КРАСНЫЙ, а не перетекание одного в другой.
      //
      // Прежняя пара «красный = t, зелёный = 1 − t» давала в середине шкалы тёмный оливковый
      // (128, 128): самый спорный участок выглядел самым грязным. Через жёлтый середина
      // становится ЯРЧЕ краёв, и «немного тронуто» перестаёт сливаться с «цело».
      const красный = t < 0.5 ? t * 2 : 1;
      const зелёный = t < 0.5 ? 1 : 2 - t * 2;
      карта.data[i] = Math.round(255 * красный);
      карта.data[i + 1] = Math.round(255 * зелёный);
      карта.data[i + 2] = 40;
      карта.data[i + 3] = 255;
    }
  ...
  _texdiffFor(родной: THREE.Material | null, эталон: SlotMaps, ключ: string) {
    ...
    if (готовая) return this._картой(готовая);
    ...
    if (!diff) return this._стеклоДиффа();
    this._diffCache.set(ключ, diff);
    return this._картой(diff);
  }
  ...
  textureRefs(): DiffRef[] {
    ...
    this.model.traverse((o: MaybeMesh) => {
      ...
      out.push({ имя: Viewer._имяДетали(o as unknown as THREE.Object3D), карты });
    });
    return out;
  }

  /**
   * Имя, по которому деталь узнают в обеих моделях.
   *
   * Берётся у самого меша, а если он безымянный — у ближайшего названного родителя: у
   * многопримитивного меша имя носит узел, а куски остаются пустыми. Пустая строка —
   * законный ответ и означает «узнать нельзя», а не «совпало с другой безымянной».
   */
  static _имяДетали(o: THREE.Object3D): string {
    let узел: THREE.Object3D | null = o;
    while (узел) {
      if (узел.name) return узел.name;
      узел = узел.parent;
    }
    return '';
  }
   ...
   */
  useDiffStore(store: Map<string, THREE.CanvasTexture>) {
   ...
   * переписано.
   */
  _сопоставитель(): (o: THREE.Object3D) => SlotMaps | null {
    ...
    if (this.model) {
      this.model.traverse((o: MaybeMesh) => {
        if (!o.isMesh) return;
        const имя = Viewer._имяДетали(o as unknown as THREE.Object3D);
        if (имя && по.has(имя)) попаданий++;
      });
    }
    ...
    if (попаданий === 0) {
      let i = 0;
      return () => refs[i++]?.карты || null;
    }
    return (o: THREE.Object3D) => {
      const имя = Viewer._имяДетали(o);
      if (!имя) return null;
      const список = по.get(имя);
      if (!список) return null;
      const n = взято.get(имя) || 0;
      взято.set(имя, n + 1);
      return список[n] || null;
    };
  }
    ...
    if (this._display !== 'file') {
      ...
      this.model.traverse((o: MaybeMesh) => {
        ...
        if (this._display === 'texdiff') {
          ...
          const эталон = эталонДля?.(o as unknown as THREE.Object3D) || null;
