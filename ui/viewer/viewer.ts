...
export class Viewer implements ViewerLike {
  ...
  _diffRaw(эталоны: SlotMaps, ставшие: SlotMaps): DiffRaw | null {
    ...
    for (let i = 0, p = 0; p < data.length; i += 4, p++) {
      ...
      const v = Math.round(сумма / пары.length);
      data[p] = v;
      всего += v;
      if (v > max) max = v;
    }
   ...
   *
   * Степень 0,4 остаётся: замер (`_work/diff-hist.mjs`) показал, что даже на самом низком
   * качестве WebP 93% пикселей отклоняются меньше чем на 5%, и без подъёма нижней части
   * диапазона весь реальный ущерб тонул в тусклой зелени.
   *
  ...
  _diffColor(сырое: DiffRaw): THREE.CanvasTexture | null {
    ...
    for (let p = 0, i = 0; p < data.length; p++, i += 4) {
      const t = Math.pow(data[p]! / 255, 0.4);
