...
export class Viewer implements ViewerLike {
   ...
   * показана, а окно — нет.
   */
  _diffCache = new Map<number, THREE.CanvasTexture>();
  /**
  ...
  _texdiffFor(mesh: THREE.Mesh, родной: THREE.Material | null, номер: number) {
    ...
    const готовая = this._diffCache.get(номер);
    ...
    this._diffCache.set(номер, diff);
   ...
   */
  useDiffStore(store: Map<number, THREE.CanvasTexture>) {
