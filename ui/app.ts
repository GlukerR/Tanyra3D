...
(() => {
  ...
  // Сколько раз ЧЕЛОВЕК трогал настройки сборки. Растёт только от его действия: правка
  // флажка, смена площадки, смена движка. Программные перестановки флажков (режим
  // «Советуем» переставляет их под каждую модель сам) счётчик НЕ двигают — они не выбор.
  //
  // Зачем счётчик, если есть подпись настроек. Подпись отвечает на вопрос про ОДНУ
  // модель — ту, что на экране, — и для неё она точнее счётчика. Но пакету нужно
  // сравнить настройки СОСЕДНЕЙ модели, а в режиме «Советуем» у каждой они свои: у одной
  // Draco, у другой ничего. Сравнивать подпись соседа с подписью того, кто сейчас на
  // экране, — сравнивать разное, и все соседи вечно выглядели бы «изменившимися».
  // Счётчик от содержимого моделей не зависит и отвечает ровно на нужный вопрос:
  // «человек что-нибудь менял с тех пор, как это собиралось?»
  let settingsRevision = 0;
  let builtRevision: number | null = null;
  ...
  const PER_MODEL_STATE = [
    ...
    { key: 'builtRevision', get: () => builtRevision, set: (v: any) => { builtRevision = v; } },
  ...
  function onOptionChanged() {
    settingsRevision += 1;      // человек тронул флажок — собранное устарело
  ...
  engineSelect.addEventListener('change', async () => {
    settingsRevision += 1;      // другой движок — другой план сборки, собранное устарело
    updateEngineDescription();
  ...
  platformSelect.addEventListener('change', async () => {
    settingsRevision += 1;      // другая площадка — другие пороги и опции
    updatePlatformDescription();
  ...
  function forgetResult(rec: ModelEntry) {
    ...
    for (const key of ['lastResult', 'lastExplain', 'lastFail', 'currentSourceId',
      'lastBuildSignature', 'builtRevision', 'resultInspect', 'resultDownloadUrl', 'resultExportBase']) {
      rec.state[key] = null;
    }
  }
  ...
  /**
   * Состояние сборки модели. У АКТИВНОЙ правда живёт в переменных, в снимок она попадает
   * только при уходе с модели — читать для неё снимок значит читать вчерашнее. Тот же
   * довод, по которому так поступают renderModelList и summaryRows.
   */
  function buildStateOf(rec: ModelEntry) {
    const live = rec.id === activeModelId;
    return {
      signature: live ? lastBuildSignature : rec.state.lastBuildSignature,
      revision: live ? builtRevision : rec.state.builtRevision,
    };
  }
   ...
   *
   * Александр, 2026-08-26: «при прогоне оптимизации прогоняться должна только новая или
   * новые добавленные, если остальные уже лежат оптимизированными… но они ведь у нас уже
   ...
   * «собиралась ИМЕННО ЭТИМИ настройками»: подпись сборки (платформа + флажки + режим
   * KTX2 + качество WebP) сверяется с текущей. Поэтому:
   *
   *   · тронул флажок — все отмеченные снова идут в работу, ни одна не «застревает»
   *     готовой (иначе замер с новой настройкой был бы враньём — Правило 12);
   *   · ничего не менял, добавил файл — соберётся только он.
   *
   * Это ровно то же правило, по которому одиночная кнопка гаснет после сборки и оживает
   * от первой же правки настроек (см. updateRunButtonState). Одна мера на оба случая —
   * иначе пакет и одиночка разошлись бы в понимании слова «уже собрано».
   *
   * Упавшая сборка готовностью НЕ считается: файла-результата нет, показывать нечего,
   * и пропустить её значило бы молча согласиться с отказом.
   */
  function needsBuild(rec: ModelEntry) {
    const { signature, revision } = buildStateOf(rec);
    // Подпись пуста в трёх случаях сразу: модель не собиралась, сборка не удалась, или
    // результат потеряли (forgetResult — файл исчез с диска). Во всех трёх показывать
    // нечего, и пропускать нечего.
  ...
  function renderResult(data: Record<string, any>) {
    ...
    builtRevision = settingsRevision;   // на какой правке настроек это собрано
