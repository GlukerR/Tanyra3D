...
(() => {
  ...
  function renderIssues(findings: ReportEntryDto[] | null | undefined, applied: ReportEntryDto[] | null | undefined) {
    ...
    if (lossyLines.length) {
      // НЕ ОШИБКА И НЕ КРАСНОЕ. Слово Александра 2026-09-03: «зачем писать как ошибку
      // красную, что необратимые изменения случились? Это ведь и так понятно, что
      // объединены и всё остальное. Модель почти никогда не будет такой же после нашего
      // приложения».
      //
      // Он прав. Необратимость здесь — не сбой, а ЗАКАЗАННОЕ действие: человек сам поставил
      // галочку, прочитав, чем платит (Правило 11 — цена называется ДО нажатия, у самой
      // галочки). Красный цвет означает «что-то пошло не так» и в этом месте просто врёт, а
      // враньё красным дороже обычного: рядом стоят настоящие отказы площадок, и человек
      // перестаёт отличать одно от другого.
      //
      // Перечень остаётся: он отвечает на другой вопрос — ЧТО именно не вернуть. Но
      // отвечает спокойно, как справка.
  ...
  //
  // Подпись называет число И ГОВОРИТ, ЧТО С НИМ МОЖНО ДЕЛАТЬ: у одной модели «нажмите, и
  // модель откликнется», у другой «только обводка — в этом графе есть незнакомые узлы».
  // Разница настоящая, и молчать о ней нельзя: человек нажал бы на обведённую часть,
  // ничего не получил и решил бы, что модель сломана.
  ...
  // Долгая работа во вьюпорте — тот же кубик, что при загрузке и сборке.
  //
  // Александр 2026-09-01: «загрузка в 4 окне сильно тормозит всё приложение и оно выглядит
  // залагавшим… чтобы сверху прыгал кубик, который показывает загрузку». Сравнение текстур
  // считается по кадру за раз (`_runDiffQueue` во вьюпорте), поэтому листать окна и нажимать
  // кнопки можно прямо во время расчёта — кубик говорит, что работа идёт, а не что всё село.
  //
  // Отдельного показателя не заводим: у человека уже есть один язык для «идёт работа», и
  // второй пришлось бы учить заново (docs/ПРАВИЛА_ИНТЕРФЕЙСА.md — один ответ в одном месте).
  // ПОДПИСКИ СТАВЯТСЯ, КОГДА ЕСТЬ НА ЧТО ПОДПИСЫВАТЬСЯ.
  //
  // Этот файл подключён обычным <script>, а вьюпорт — модуль, то есть отложен и приходит
  // ПОЗЖЕ. Прямой вызов `window.OptiViewer?.setOnBusy?.(…)` отсюда попадал в `undefined`, и
  // `?.` глушил его молча: ни ошибки, ни подписки. Так были потеряны обе — и кубик долгой
  // работы, и запись о нажатии на интерактивную часть (найдено 2026-09-03).
  //
  // Мост `onOptiViewerReady` объявляет вьюпорт (ui/viewer/index.ts, конец файла). Вызов
  // ниже — на случай обратного порядка: если лицо уже объявлено, ждать нечего.
  ...
  // ---------------------------------------------------------------
  // ЧЕТВЁРТЫЙ СПОСОБ ПОКАЗА — ЗА ГАЛОЧКОЙ В НАСТРОЙКАХ
  //
  // Александр 2026-09-05: «основными режимами были только 3, а четвёртый только при
  // включении его в настройках… Это тяжёлый мод и он не должен быть для всех и для всего.
  // на слабых пк он вообще не пойдёт».
  //
  // Он прав по обеим причинам. По СМЫСЛУ: первые три отвечают на вопрос «как выглядит эта
  // модель» и работают на любой, четвёртый — на другой вопрос, «что в ней изменила
  // сборка», и без второй модели он бессмыслен. По ЦЕНЕ: он читает КАЖДУЮ карту обеих
  // моделей пиксель за пикселем и считает по ним схожесть структуры — на тяжёлой модели
  // это десятки мегабайт и заметные секунды даже на сильной машине.
  //
  // ВЫКЛЮЧЕНО — ЗНАЧИТ КНОПКИ НЕТ. Не погашена: погашенная кнопка без объяснения читается
  // как поломка, а показанная обязана работать (Правило 12). Объяснение живёт там, где
  // человек её включает, — в настройках, рядом с галочкой.
  //
  // Выбор помнится между запусками, как язык (`tanyra.lang` в ui/i18n.ts): человек,
  // включивший тяжёлый режим однажды, не должен включать его каждое утро.
  ...
  /**
   * Показать или убрать четвёртую кнопку.
   *
   * Снятая галочка ещё и ВОЗВРАЩАЕТ показ к материалам файла, если человек стоял в этом
   * режиме. Иначе он остался бы в режиме, кнопки которого больше нет: выйти нечем, а
  ...
  if (texdiffSetting) {
    texdiffSetting.addEventListener('change', () => {
      ...
      try { localStorage.setItem(ТЯЖЁЛЫЙ_РЕЖИМ, on ? '1' : '0'); } catch (e) { /* приватный режим */ }
      применитьTexdiff(on);
    });
  }
  применитьTexdiff(texdiffРазрешён());

  // НАСКОЛЬКО ВООБЩЕ ИЗМЕНИЛОСЬ — числом, рядом с картинкой. Мера — SSIM (схожесть
  // структуры), общепринятая и понятная без пояснений: 100% — структура цела.
  //
  ...
  function refreshDiffScale() {
    ...
    // `доля` — схожесть структуры худшей детали: 1 — цела. Единица означает «сравнили и не
    // нашли ничего», и подписывается отдельно: «сохранено на 100%» звучало бы как отчёт о
    // проделанной работе там, где работы не было.
  ...
  window.onOptiViewerModelLoaded = () => {
    ...
    // Подпись о схожести — тоже про модель, а не только про расчёт. Без этой строки после
    // пересборки она оставалась от прежней сборки, пока человек не переключит режим
    // туда-обратно: расчёт заканчивался и сообщал о себе, но между «модель приехала» и
    // «расчёт кончился» на экране висело старое число.
    refreshDiffScale();
  };
