...
(() => {
  ...
  // Отпечаток настроек, которые человек задал ДЛЯ ВСЕХ МОДЕЛЕЙ СРАЗУ. Им меряется
  // готовность СОСЕДНИХ моделей — тех, чьих флажков сейчас не видно.
  //
  // Здесь стоял СЧЁТЧИК правок, и он был неверен. Счётчик считает нажатия, а не разницу:
  // поставил флажок и снял обратно — две правки, настройки прежние, а весь пакет уходил
  // на пересборку. Александр поймал это сразу, 2026-08-26: «я выбираю новый флажок и
  // потом его убираю. и модели снова все нужно полностью оптимизировать, хотя в левом
  // вьюпорте уже висит оптимизированная по этим же параметрам модель». И тут же назвал
  // образец: «одна модель такую ошибку не ловит» — у одиночной кнопки сравниваются
  // ЗНАЧЕНИЯ, и возврат флажка гасит её обратно.
  //
  // Значит и здесь надо сравнивать значения. Что входит в отпечаток — зависит от режима,
  // и это не тонкость, а суть:
  //
  //   · «Мой выбор» — флажки ОБЩИЕ для всех моделей (restoreSelection берёт их из
  //     savedSelections площадки). Значит подпись настроек входит в отпечаток целиком.
  //   · «Советуем» — флажки выводятся ИЗ САМОЙ МОДЕЛИ заново при каждом её показе
  //     (showActiveModel → applyDetection → applyDefaultSelection). Правка флажка до
  //     соседей просто не доезжает: их панель будет собрана заново. Поэтому в отпечаток
  //     входит только то, что действительно общее, — площадка, движок и сам режим.
  //
  // Для модели НА ЭКРАНЕ отпечаток не нужен вовсе: её флажки видны, и needsBuild
  // сравнивает подписи напрямую — тем же способом, что одиночная кнопка.
  function settingsStamp() {
    return `${platformSelect.value}|${engineSelect.value}|${adviceMode}`;
  }

  let builtStamp: string | null = null;
  ...
  const PER_MODEL_STATE = [
    ...
    { key: 'builtStamp', get: () => builtStamp, set: (v: any) => { builtStamp = v; } },
  ...
  function forgetResult(rec: ModelEntry) {
    ...
    for (const key of ['lastResult', 'lastExplain', 'lastFail', 'currentSourceId',
      'lastBuildSignature', 'builtStamp', 'resultInspect', 'resultDownloadUrl', 'resultExportBase']) {
      rec.state[key] = null;
    }
  }
   ...
   *
   * ОТВЕТ РАЗНЫЙ ДЛЯ ЭКРАНА И ДЛЯ СОСЕДЕЙ, и это не двойной стандарт, а разное знание.
   *
   *   · Модель НА ЭКРАНЕ. Её флажки видны — сравниваем ПОДПИСИ, точно. Это ровно то,
   *     чем живёт одиночная кнопка (updateRunButtonState), и потому пакет с одиночкой
   *     не расходятся в понимании слов «уже собрано».
   *   · СОСЕДИ. Их флажков на экране нет. В режиме «Советуем» они у каждой свои и будут
   *     выведены заново при показе, поэтому сравнивать их подпись с текущей нельзя —
   *     все соседи вечно выглядели бы изменившимися. Сравниваем ОТПЕЧАТОК общих
   *     настроек (settingsStamp).
   *
   * Почему не счётчик правок, который стоял здесь до этого: он считает НАЖАТИЯ, а не
   * разницу. Поставил флажок и снял обратно — настройки прежние, а счётчик уехал, и
   * весь пакет уходил на пересборку впустую. Значения сравнивать обязательно.
   *
  ...
  function needsBuild(rec: ModelEntry) {
    ...
    const signature = live ? lastBuildSignature : rec.state.lastBuildSignature;
    if (signature == null) return true;
    // Точный ответ там, где известны будущие флажки модели: на экране они видны, а в
    // режиме «Мой выбор» они ОБЩИЕ — соседу достанется ровно то, что стоит сейчас.
    if (live || adviceMode === 'manual') return signature !== currentSettingsSignature();
    // «Советуем»: флажки соседа будут выведены ИЗ НЕГО САМОГО при показе. Сравнивать
    // тут нечего, кроме общего — площадки, движка и самого режима.
    return rec.state.builtStamp !== settingsStamp();
  }
  ...
  function renderResult(data: Record<string, any>) {
    ...
    builtStamp = settingsStamp();   // при каких общих настройках это собрано
