...
interface OptiViewerApi {
  ...
  /**
   * Части, откликающиеся на нажатие, и показана ли их обводка.
   *
   * `playable` — проигрывает ли окно граф поведения у ЭТОЙ модели. Не всегда: встретив
   * незнакомый узел или адрес, вычислитель отказывается целиком, и тогда остаётся одна
   * обводка. Половинчатое проигрывание хуже отсутствия.
   */
  ...
  /**
   * Чему равен красный в режиме различий: доля 0…1 от полной яркости, `null` — сравнивать
   * было нечего. Шкала растягивается по САМОЙ модели, поэтому число обязано быть на экране:
   * без него «сломано сильнее всего в этой модели» неотличимо от «сломано вообще».
   */
