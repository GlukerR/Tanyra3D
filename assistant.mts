...
/**
 * Что в профиле площадки МЫ НЕ ПОНЯЛИ. Пустой список — понято всё.
 *
 * ЗАЧЕМ (решение 2026-09-01, вопросы 1.3 и 1.4 аудита). Профиль пишет человек, руками, в
 * своей папке — и до сегодня незнакомое в нём проходило МОЛЧА. Написал
 * `budgets.vertices: 1000` — порог не применялся ни разу, ошибки не было, и он думал, что
 * работает. Написал `by: 'нашеЧисло'` — пометка о происхождении просто не рисовалась.
 *
 * Молчание здесь хуже отказа: у проекта есть правило «нет проверки — нет совета», и
 * непонятое поле — ровно тот случай, когда мы обязаны сказать, а не додумать.
 *
 * ПОЧЕМУ ОДИН КАНАЛ НА ДВА ВОПРОСА. Изнутри это две разные ошибки, снаружи — одна: «в
 * твоём профиле есть то, чего программа не знает». Два отдельных сообщения человеку
 * пришлось бы читать как два разных случая, хотя делать в обоих одно и то же — открыть
 * файл и поправить строку.
 *
 * ПОЧЕМУ НЕ ОТКАЗ. Профиль читается на СЕРВЕРЕ, человека у экрана в этот момент нет
 * (`docs/АУДИТ_итог_предложения.md`, вопрос 1.4). Отказаться значило бы уронить площадку
 * целиком из-за одной непонятой строки — остальные её пороги при этом верны и полезны.
 * Поэтому: применяем понятое, называем непонятое.
 */
function profileUnknowns(p: Record<string, unknown>): string[] {
  ...
  for (const [key, raw] of Object.entries(budgets)) {
    // Порог, которого движок не умеет считать. Список известных — `BUDGET_SPEC`, и он
    // здесь ЧИТАЕТСЯ, а не переписывается: своя копия разошлась бы молча.
    if (!(key in BUDGET_SPEC)) { out.push(key); continue; }
    // `by` — пометка «чьё это число». Значений ровно два, и это решение 2026-09-01:
    // пометка о происхождении, которую молча проглотили, хуже её отсутствия.
  ...
  for (const { dir, custom } of PROFILE_DIRS()) {
    for (const { id, profile: p } of profileEntries(dir)) {
      ...
      out.push({
        ...
        custom,
        // Что в профиле не понято. Пустой список интерфейс не показывает никак —
        // предупреждение обязано гореть только когда есть о чём предупреждать.
