...
export interface RuleMeta {
  ...
  feature?: string;
  /**
   * Для advanced-правила, которое обслуживает ГРУППУ взаимоисключающих фич: id этой
   * группы (тот же, что в exclusiveGroups аддона).
   *
   * Заведено 2026-08-12 для `textures/resize`: у него четыре размера (4096/2048/1024/512),
   * и назвать в `feature` один из них значило бы соврать в отчёте — при пустом выборе
   * человек прочитал бы «флажок resize-2048 не включён», хотя он не выбирал ни одного.
   * Поле объявляет принадлежность правила выключателю, не называя конкретный член.
   */
  featureGroup?: string;
...
export interface Addon {
  ...
  /**
   * Сколько весит исходная модель НА САМОМ ДЕЛЕ. Необязательный хук: не объявил —
   * движок возьмёт размер файла, и для форматов, где модель это один файл, этого хватает.
   *
   * Нужен там, где файл модели — только оглавление, а данные лежат рядом отдельными
   * файлами (`.gltf` с `.bin` и папкой текстур). Без него отчёт сравнивает вес
   * оглавления с весом готовой модели и называет шестикратное уменьшение ростом.
   */
...
export interface RunResult {
  ...
  error?: string;
  /**
   * Рецепт строки `error` — тот же приём, что у записей отчёта.
   *
   * Без него причина отказа навсегда оставалась на языке, на котором шла сборка: человек
   * переключал язык, весь отчёт переезжал, а единственная строка, ради которой он в этот
   * экран и смотрит, — нет. Хуже: интерфейсу нечем было отличить «модель не прочиталась»
   * от «результат не прошёл проверку», и он подставлял вторую причину под первую —
   * ровно то смешение, которое §4b запрещает прямым текстом.
   *
   * Заполняется, когда у брошенной ошибки есть `i18n` (аддон вешает его на свои отказы:
   * облако точек, обрезанный файл, недостающие соседи `.gltf`). Нет рецепта — поля нет, и
   * `error` остаётся как есть: техническое сообщение честнее выдуманного перевода.
   */
  i18n?: I18nRefs;
}
