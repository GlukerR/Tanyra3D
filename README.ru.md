...
Хотите то, что делается прямо сейчас? [Пробные
сборки](https://github.com/GlukerR/Tanyra3D/releases) лежат на странице релизов с
отметкой **Pre-release**.
...
Кнопка «Скачать» выше всегда ведёт на проверенную сборку. Выпуски с отметкой
**Pre-release** на той же странице — это то, что пробуется сейчас: берите, если хотите
свежее, и будьте готовы к шероховатостям.
...
**0.2.2 — своя площадка целиком и порядок на диске.** Площадка теперь вычитает опции:
снятая галочка означает «эта площадка такого не читает», и опция исчезает из панели
совсем. Площадками делятся файлом — «Открыть файл…» и «Сохранить в файл». Размеры
текстур крупнее самой большой текстуры модели больше не показываются: увеличивать
программа не умеет, а кнопка, которая ничего не делает, только путает. И рабочая папка
перестала расти незаметно — потолок по объёму, уборка при выходе, а занятое место видно
в настройках.
