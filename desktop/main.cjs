...
// ПОСЛЕДНИЕ строки вывода движка — кольцом, и собираются они ВСЕГДА.
//
// Раньше сбор выключался сразу после удачного запуска: «дальше сервер работает часами, и
// держать его журнал в памяти незачем». Довод верный, а вывод из него был неверный.
// Держать незачем ВЕСЬ журнал — но не последние строки: когда движок умирает на ходу,
// именно они и есть единственное, по чему можно понять причину.
//
// Чем это было (найдено ревью 2026-08-22): человек видел «Движок остановился (код N)»,
// файла engine-crash.log при таком выходе не появлялось вовсе, а буфер в памяти к этому
// моменту был очищен. То есть о падении на ходу мы не узнавали НИЧЕГО — ни мы, ни он.
//
// Двести строк — это заведомо больше, чем занимает причина падения (обычно её видно в
// последних пяти-десяти), и заведомо мало, чтобы что-то весить.
...
if (gotTheLock) app.whenReady().then(async () => {
  try {
    ...
    // Строки запуска дальше не нужны: место в кольце дороже отдать тому, что скажет
    // движок ПЕРЕД падением. Сам сбор при этом продолжается — см. pushLog.
    serverLog.length = 0;
    ...
    serverProcess.on('exit', (code) => {
      ...
      // Отчёт пишем ЗДЕСЬ же. При запуске он писался всегда, а при падении на ходу — ни
      // разу: человек оставался с кодом выхода и ничем больше, и рассказать о поломке
      // ему было нечем.
      const file = saveCrashLog();
